...
class Postava {
    constructor(jmeno, rasa, maxHp, sila, obratnost, inteligence) {
        this.bonusMaxHp = 0; // Nový atribut pro bonusové maximální životy z vybavení
...
class Bojovnik extends Postava {
    ...
    zmenAdrenalin(hodnota) {
        this.redukcePoskozeni += hodnota;
        if (this.redukcePoskozeni > 80)
            this.redukcePoskozeni = 80; // Limit na max 80% redukce
        if (this.redukcePoskozeni < 0)
            this.redukcePoskozeni = 0;
    }
...
class Jidlo extends Predmet {
    constructor(nazev, popis, silaLeceni) {
        super(nazev);
        if (silaLeceni < 0)
            throw new Error("Síla léčení z jídla nesmí být záporná.");
        this.popis = popis;
        this.silaLeceni = silaLeceni;
    }
    getPopis() {
...
class Vybaveni extends Predmet {
    constructor(nazev, typ, silaBonus, rychlostUtoku, inteligenceBonus, obranaBonus, obratnostBonus) {
        super(nazev);
        this.typ = typ;
    ...
    getSilaBonus() { return this.silaBonus; }
    getRychlostUtoku() { return this.rychlostUtoku; }
    getInteligenceBonus() { return this.inteligenceBonus; }
    getObranaBonus() { return this.obranaBonus; }
    getObratnostBonus() { return this.obratnostBonus; }
}
// ==========================================
...
const inputJmeno = document.getElementById("input-jmeno");
const selectRasa = document.getElementById("select-rasa");
const selectPovolani = document.getElementById("select-povolani");
...
let vybavenaZbran = null;
let vybavenyStit = null;
// Globální proměnné pro aktuálně zvolenou rasu a povolání
let zvolenaRasaNazev = "";
let zvolenePovolani = "";
...
function aktualizujStavUI() {
    ...
    if (uiSila)
        uiSila.innerText = celkovaSila.toString();
    if (uiInteligence)
        uiInteligence.innerText = celkovaInteligence.toString();
    if (uiObratnost)
        uiObratnost.innerText = celkovaObratnost.toString();
...
function pridejPredmet(hledanyNazev) {
    ...
    if (hledanyNazev.includes("|")) {
        ...
        if (type === "potion" && parts.length >= 4) {
            ...
            if (pType === "zdravi")
                inventar.push(new LektvarZdravi(itemName, pVal));
            else
                inventar.push(new LektvarMany(itemName, pVal));
            return;
        }
    ...
    if (lektvar) {
        if (lektvar.typ === "zdravi")
            inventar.push(new LektvarZdravi(lektvar.nazev, lektvar.hodnota));
        if (lektvar.typ === "mana")
            inventar.push(new LektvarMany(lektvar.nazev, lektvar.hodnota));
        return;
    }
    ...
    if (vybaveni) {
        inventar.push(new Vybaveni(vybaveni.nazev, vybaveni.typ, vybaveni.modSila || 0, vybaveni.modRychlostUtoku || 0, vybaveni.modInteligence || 0, vybaveni.modObrana || 0, vybaveni.modObratnost || 0));
        return;
    }
...
if (btnOdeslat && inputChat && chatLog) {
    btnOdeslat.addEventListener("click", async () => {
        ...
        if (hrdina) {
            ...
            let zdrojText = "";
            if (hrdina instanceof Mag)
                zdrojText = `Mana: ${hrdina.getMana()}/100`;
            else if (hrdina instanceof Zlodej)
                zdrojText = `Focus: ${hrdina.getFocus()}/100`;
            else if (hrdina instanceof Bojovnik)
                zdrojText = `Adrenalin (redukce zranění): ${hrdina.getRedukcePoskozeni()}%`;
            let pravidloZdroj = "";
            if (hrdina instanceof Mag)
                pravidloZdroj = "PRAVIDLO PRO MANU: Jsi Mág. Tvá Mana se spotřebovává POUZE při sesílání magických kouzel. Fyzické akce manu nestojí!";
            else if (hrdina instanceof Zlodej)
                pravidloZdroj = "PRAVIDLO PRO FOCUS: Jsi Zloděj. Tvůj Focus se spotřebovává POUZE při náročných, přesných nebo rychlých akcích (kradení, uhýbání, akrobacie).";
            else if (hrdina instanceof Bojovnik)
                pravidloZdroj = "PRAVIDLO PRO ADRENALIN: Jsi Bojovník. Tvůj Adrenalin stoupá POUZE v boji, když dostaneš zásah od nepřítele.";
                        ...
                        if (commandString.toLowerCase() !== "none") {
                            ...
                            for (let cmd of commands) {
                                ...
                                if (cmd.toLowerCase().startsWith("hp=")) {
                                    ...
                                    if (!isNaN(resVal)) {
                                        if (hrdina instanceof Mag)
                                            hrdina.zmenManu(resVal);
                                        else if (hrdina instanceof Zlodej)
                                            hrdina.zmenFocus(resVal);
                                        else if (hrdina instanceof Bojovnik)
                                            hrdina.zmenAdrenalin(resVal);
                                    }
                                }
                            }
                            aktualizujStavUI();
